...
## Round 7 — 2026-07-31 (authorship + external rescout)

### CLOSED (feasibility) — Computational stylometry of solved-page prose for authorship
**Mechanism:** fingerprint the solved LP English plaintext (function words, syntax, register) and attribute against candidate-author corpora.
**Why closed:** the entire solved LP English corpus is **~800 words**, far below the 2,000–5,000-word reliability floor (Eder 2015; Koppel 2009); positive attribution on this sample is manufacturable by chance (false-attribution >60% under ~3k words) — the textual analogue of an un-anchorable keyspace search. NOT feasible with existing text. **Do not revive** as a positive-attribution method unless the solved corpus grows past ~2,000 words (i.e., many more LP2 pages get solved). A NEGATIVE-SPACE clustering/exclusion run is the only defensible use now, and only as EXPLORATORY. The stronger authorship signal is crypto-design taste (see identity/verification-ledger.md), which stays at MODERATE and names no individual.

### QUARANTINED (scam/distractor) — `53cr37-layer` "solve" → Solana token
Copycat `unc0vering-cicada` fan wiki claims LP2 p1–73 decrypt (keys DANAD/MIDBV from the "Know This" square) to a Solana token address + Chechen "CICADA3301" LLC. **HIGH-confidence crypto-scam:** no 3301 PGP signature; keys are ciphertext-derived (not external); "plaintext" is a token address that postdates LP; zero-provenance copycat host. **Do not chase.** Not a solve, not an external input.

### REAFFIRMED NOISE — Zenodo "IA Gemini Final Translation" (Becker/OFFELLIA, 2026)
Unauthenticated AI-assisted ciphertext-only personal translation; no PGP signature, no delivered key. Same class as the R4-logged Zenodo noise. Not an external input.

### NO NEW EXTERNAL INPUT (2025–2026 sweep) — cipher stays legitimately closed
Full sweep (CicadaSolvers/GitHub/Zenodo/wiki/DEF CON/Reddit/Tor-archive) found nothing clearing the bar. Last verified 3301 PGP-signed message remains April 2017; `gy3hoy2zizvuzvdb.onion` still never archived. **Do not reopen ciphertext attacks** until a new signed artifact/key surfaces.

---

## Round 8 — 2026-08-06 (security/forensics lane, on canonical origin/master base)

### KILLED (Gate #2, executed then NEGATIVE) — S1 "ICC-profile interior generation-tooling fingerprint"
**Mechanism:** the embedded APP2/ICC profile (never structurally parsed before — prior work only recorded
`has_icc=true`) might carry a Ghostscript build-version string or an ICC creation timestamp that dates the
render or fingerprints the operator's toolchain.
**Why dead:** parsed (`research/experiments/r8-01-icc-interior-parse/`). The profile is byte-identical
across all 56 pages and is the **stock minimal Artifex sRGB profile** Ghostscript ships: `creation_datetime`
all-zero, NO manufacturer/model/creator/profile-ID, NO build-version tag, only `desc="Artifex Software sRGB
ICC Profile"` + `cprt="Copyright Artifex Software 2011"` (the profile's own static copyright, NOT a build
date). Per the pre-registered decision token (which excludes the static 2011 copyright) → NEGATIVE. It
CONFIRMS the Ghostscript/Artifex renderer structurally (a small provenance upgrade) but yields no new
dating, no operator slip. `platform=APPL` is hardcoded in the stock profile, NOT a Mac-operator signal.
**Do not revive** without ORIGINAL-distributed artifacts carrying different metadata than the local set
(the local set is already SHA1-matched to the archived onion7 dump, so this is unlikely to differ).

### KILLED (Gate #1, pre-execution) — Lateral provocations P1–P5
- **P1 doublet-POSITION discriminator (inter-doublet gap distribution):** re-run of R5-S4 (TIDELINE), which
  already measured inter-doublet gaps as geometric/memoryless (KS p=0.69). English-vs-filler-under-OTP is
  additionally foreclosed by the master lineage's "message-existence UNDECIDABLE" (both bands overlap on all
  10 statistics). No new mechanism.
- **P2 inter-ᚠ interrupter gap-LENGTH sequence as plaintext channel:** re-skin of the closed F-channel family
  (R1-H1 interrupter-as-doublet-breaker; R5-DELVE placeholder channel; F-mask→ASCII; interrupter-masked
  running key — all null). Underpowered (458 F / 12,956 runes) and reintroduces the forbidden English-prose
  null as a discriminator.
- **P3 pp49-51 payload as a 2D pad (~20 traversals):** self-rated 5%; 256 bytes ≈ 70 runes is underpowered
  to score English; pp49-51 is already exhausted as key/text/number/format/XOR/image across master Campaigns
  VII/IX/XI/XII and is on the "Do NOT re-run" list.
- **P4 line-LENGTH channel (rune-count-per-line compressibility / number-sequence match):** the `/`-line
  channel is a known FALSE-POSITIVE generator (master Campaign XIV retracted a `/`-driven word-length signal
  with sign OPPOSITE to English); the "matches any number sequence" arm is uncontrolled multiple-comparisons
  and number-theoretic sequences are already dead.
- **P5 words-per-line ratio solved-vs-unsolved:** redundant with Campaign XIV P5 (word-length already found
  English-shaped, with the `/` false-positive caveat) and cannot beat the message-existence-undecidable
  result.
**Do not revive** any of P1–P5 without a NEW mechanism clearing the specific revive-bar of the family it
re-skins.

> **Note — S2 (DQT page-membership) is NOT a dead end.** It returned **INCONCLUSIVE** (a reproducible
> positional pattern, batch-vs-content unresolved). It lives in `OPEN_QUESTIONS.md` with a pinned resolving
> measurement, not here. It has ZERO ciphertext bearing regardless of how it resolves.
