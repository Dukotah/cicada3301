#!/bin/bash
# Re-clone every target whose vendor dir is not a valid, checked-out git repo.
# Valid = .git present, HEAD resolves, and at least one non-.git entry on disk.
...
| while IFS='|' read -r d u dep; do
  ...
  if [ "$dep" = "full" ]; then
    ...
    timeout 150 git clone --quiet --depth "${dep:-20}" "$u" "$V/$d" >>"$E/clone.log" 2>&1
  fi
  if valid "$d"; then
