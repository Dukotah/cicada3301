...
1b. **DISCHARGED (Round 10) — corrected stylometric-exclusion test.** Done. The corrected run (impostor
   false-inclusion / true-exclusion-power gate) executed and returned a decision-grade **NEGATIVE**:
   true-exclusion power 0.20–0.27 at N_q≈424–729, no viable ROC operating point → LP exclusions uninterpretable
   at this corpus size. **Stylometry lane now CLOSED in both halves** (see `DEAD_ENDS.md` R10). The sibling
   campaign at `liber-primus/analysis/stylometry/` (2026-07-13) is on record for future rounds.
2. ~~**(EXPLORATORY only) Negative-space stylometric exclusion**~~ — **CLOSED (Round 10);** measured
