...
| **Round 18** | **Aim at the instrument (8 lanes, 2026-08-25/26)** | **Round 18 — AIM AT THE INSTRUMENT (2026-08-25/26).** Eight pre-registered lanes, every one with a positive control; **0 decodes over bar**, and the most consequential round since D3 — because four lanes found the *instruments*, not the search space, to be the limit. **L7 (FOUND-ERROR ×3):** the word "English" appears in no `coverage` field of the ledger, yet the repo's own instrument, handed a **correct key**, recovers 100% of rune indices over Latin/Welsh/vowel-dropped-English plaintext and scores it as noise — worst case **below a wrong key**. Every negative here is an **English-register** negative; B-04, R16-KDF, R16-PRNG, R17-PUBLIC-PAD and B-21 carry restated coverage (originals preserved in `restated_from`). It does **not** void them: the register LP2 uses scores −4.33 at power 1.00. Also: the beam represents exactly **one** rejection-loop implementation (a two-draw sampler reproduces LP2's doublet rate and is missed entirely), and `threshold_for()` was used outside its calibrated domain — **R17's headline −5.679 was the null's own maximum**. **L2:** the filter acts on the **ciphertext** (alternatives excluded at power 1.000), drift is **373.6 ± 19.6** draws, the SAT route is proved dead, and the filter *is* a leak **on the key** — a **language-free** screen at 3.08 sd. **L1:** the images are a two-stage `gs` → ImageMagick pipeline, `gs` bounded to **9.04–9.14**; ranked generator prior. **L5:** the 6 contested payload bytes resolved at 100% calibrated accuracy (none change) — and **3 real errors** found elsewhere in `canon_256.bin`. **L6:** 2.04×10⁹ offsets, Marsaglia CDROM hash-verified and swept for the first time. **L3:** 109 bands / 9,899 glyphs read, uncovering **256 non-runic tokens the analysis tree never held**. **L4:** C-02's *specified* detector has **FPR 0.490** — running it as written would likely have announced a false discovery. **L8:** the PGP verification table the repo never had (54/54 verify, one key) plus **433 ranked seed candidates**. | `analysis/round18/SYNTHESIS.md` |
| **Round 19** | **Repair the instrument (14 lanes, 2026-08-26)** | **Round 19 — BUILD THE NEW MAGNET, THEN FIND IT CAN'T AFFORDABLY SWEEP.** 14 pre-registered lanes; **0 decodes over bar** (no sweep was released). **I1** repaired the decoder (drift-tolerant beam, preset lam=12/max_free=2 recovers 20/20 L7-B failure cells the repo scored 0/20; `keyskip1` bit-identical to the repo beam). **I2** built a nine-register adjudicator (power ≥0.90 in 27/27 cells; Latin 0.33→1.00, Welsh 0.00→1.00) but `EN_NOVOWEL` recovery only 67–81% (a hit there is detection, not readable plaintext). **I3** recalibrated the nulls (`calib19.json`; the −5.5 bar was wrong in 14/14 historical sweeps, usually too strict; **no verdict flips**), and named two open items: a **G-PANEL failure** (needs a direct panel-max null) and the **`n_skips` null curve** as the highest-value single addition. **R1** (red-team) found the round could **not affordably sweep**: the only cheap prefilter is `round17`'s rigid, English-only `dense_scan` — the two defects the new instrument exists to repair — so Round 19 correctly did **not** run Phase 2. **G1–G4** built byte-exact generators (bash `$RANDOM`, Perl 5.14, Python 2.7, TeX LCGs) but all **HELD at the scoring boundary, 0 coverage**. **T1/T2/T3** upheld canon **450/450** from three directions (indels ≤1 at 95%, E[W]=0), found the decode channel fragile but `n_skips` robust (98.4% of margin survives k=450 substitutions). **C1** corrected **3 payload bytes** (idx 45/50/246 → `payload_resolved.bin`, voiding B-05 which re-ran clean). **C3** re-derived the PGP table (228/228 identical to R18), **corrected L1's F6/F7** (56 files / 54 messages / to 2017; toolchain changed twice) and surfaced the **Crypt::RSA** Perl fact. Ledger merged to LEDGER.json (41 new entries, 10 restated). | `analysis/round19/` (per-lane `RESULTS.md`); `LEDGER.json` |
| **Round 20** | **Build the sieve, sweep the enumerable haystack — sieve infeasible (10 lanes, 2026-08-28)** | **Round 20 — THE SIEVE IS INFEASIBLE; THE P-GATE HELD.** Five phases; the load-bearing lane **P1** (a skip-aware, multi-register prefilter) is the whole round's precondition — and it **could not be built**: best simultaneous survival at ≥100× reduction across CY + half-vowel English = **0.667** (Wilson95 upper bound 0.85 < the 0.90 bar) → **INFEASIBLE**. Because Phase S was hard-gated on P1∧P2∧P3, and P1 failed, **no S-lane scored a decode** — the enumerable PRNG sweeps (Python 2.7, bash `$RANDOM`, Perl, TeX, Marsaglia) remain **built-but-unswept, 0 coverage**, exactly as Round 19 left them. **P2** built the `n_skips` null curve (`nskips_null.json`, seed 3301) + wired `SWEEPROW/2`, but the third deliverable fell short. **P3** PASSED all three sub-tasks: the **panel-max direct null** (`panelmax20.json`, correct-key power 1.00 / wrong-key 0.00 on LP1_REAL/LATIN/OE/half-vowel), the **seed prior** (`seedprior20.json`, 433 candidates, rank 1 = 1325734783), and the **drift re-baseline** (OE `pmax` shortfall confirmed 13.543 vs 13.842; fragile-decode-channel bound holds). **R (red-team)** returned **NO-ERROR-FOUND** on the structured-null power-leak attack (structured wrong-key survival 0.000) but **FOUND-ERROR (latent)** on sieve × panel-max interaction: screened-survivor panel-max FP **0.025/decode** vs nominal 1e-8 → a **2.5×10⁶ inflation** any future sieve must correct for. **N1** — **NEGATIVE**: no surviving LP PDF/PostScript with a runic text-layer or embedded font subset in 195 local PDFs or across the four named mirrors; the S-TEX `allrunes` promotion trigger did **not** fire. **N2** — **NO-ERROR-FOUND**: ~20 doublet-site haplographic merges do **not** reopen autokey (refutation hardened). **C (closeout)** propagated the L1-prior corrections to source (F6/F7 counts, looser distro bracket, G2→Crypt::RSA) and filed this round's nav + `PICKUP-HERE.md`. **The standing verdict is unchanged:** LP2 0–54 is OTP-class; what Round 20 moved is the *measured* status of the "cheap seeded 2012-Linux PRNG" branch — the sieve that would make it affordable to sweep is now shown infeasible at the ≥100×/0.90 target, so those families stay bounded-but-unswept, not closed. | `analysis/round20/` (per-lane `RESULTS.md`); `analysis/round20/C/RESULTS.md`; `PICKUP-HERE.md` |
| **Round 21** | **Seal the gate, finish the Py2.7 reducer axis, measure the n_skips crossover (4 lanes, 2026-08-29)** | **Round 21 — THE NO-ORACLE SEAL IS PROVABLY LEAKY; ONE MORE PRNG SLICE IS CLEAN; THE n_skips WINDOW IS ~6000.** Four lanes, standing OTP-class verdict unchanged. **L1 (seal, KILL):** the no-oracle disjoint-fold proxy that Round 20's red-team flagged as leaky *improves* the catch of correct-key vowel-dropped-English hallucinations enormously (single-cut 0.33 → best fold 0.80 genuine-safe / 0.87 ceiling) but **cannot reach the pre-registered 0.90** on its own tuning-45, and on a fresh held-out surrogate the false-reject drifts to **0.167** (over the 0.10 cap). No fold cell is both ≥0.90 catch and ≤0.10 false-reject. **Consequence: Round 21 HIT auto-certification is WITHHELD — any bar-clearing survivor is flagged-for-oracle, never auto-certified.** The seal stays open to a *non-fold* proxy family (longer decrypt head, per-register bar, learned self-consistency). **L3 (Py2.7 reducers + amd64 map, CLEAN NEGATIVE):** 884,000 words screened across `grb5_mod`/`grb5_rej`/`shuffle29` (i386) + `random29` (amd64 2-word init) — the three reducers + the 64-bit ABI map S-G3 never covered — **0 hits; 4/4 positive controls recover at recovery 1.000; best decode pmax 6.248 vs the 7.634 bar** (1.39 below). The 45,975-word prior-dense front is fully covered per config. **BOUNDED, not closed:** >99.99% flat-prior tail per config (~61 core-days total), the full 2⁶⁴ amd64 image, and inherited S-G3 gaps (other decoder relations, non-panel registers, PYTHONHASHSEED/WichmannHill/Py2.6, `/dev/urandom`) all remain. **L2 (instrument, n_skips crossover):** on the exact `keyskip1` relation the `n_skips` statistic crosses from non-separating to separating at **L\*=6000** (bracket (3600,6000], holds at 9000/12956); correct-key overtakes the wrong-key *mean* at L≈1000 but clears the exact 1% right *tail* only at 6000 → **any whole-book / concatenated-page n_skips adjudication needs a window ≥ ~6000 runes before n_skips alone is valid**. The `drift` channel's apparent "separation" is a LEFT-tail artifact (wrong-key skip inflation), not a discriminator; `skip_by_two` (R18 L7-B) is beam-unrepresentable (recovery collapses, crossover None). **L5 (red-team, MIXED):** independently confirms the L1 KILL; and returns **NO-ERROR-FOUND** on (b) no silent re-run of foreclosed lanes, (c) the 2.5×10⁶ sieve × panel-max FP inflation is NOT re-introduced (fold machinery is FP-neutral, Round-21 sweeps unscreened), (d) the row gate is honest (`hitfn20.is_hit` requires clears-null ∧ recovery≥0.90 ∧ held-out≥0.90 — score alone can never be a HIT). The FP-inflation conditional stands exactly as R left it in Round 20, binding on any FUTURE screened sieve only. | `analysis/round21/` (per-lane `RESULTS.md` + `ledger.json`); `LEDGER.json` (R21-L1/L2/L3/L5); `PICKUP-HERE.md` |
| **Round 22** | **Read the four signed-hint channels for the first time (5 lanes, 2026-08-29)** | **Round 22 — THE HINTED CHANNELS ARE READ AT LAST; ALL FOUR CLEAN, CONTROL-VALIDATED NEGATIVES.** After Rounds 16–21 spent almost their whole budget on the letter-stream PRNG/seed branch, Round 22 finally coded and read the channels the *signed hints* point at — none of which had ever been run as data. **A (X2, self-embedded read, NULL):** 3,508 selection functions (every-k / reversed / diagonal folds) over the 5 rig-solved pages' DECRYPTED plaintext (not the ciphertext); Phase-0 planted-acrostic recovery **1.000**; **0 family-wise survivors** (6 per-cell FPs vs 3.5 expected by chance — noise); language-agnostic secondary panel flat. 'Seek within / test the knowledge' as an acrostic-of-the-solution is clean-negative. **B (G1-turtle + G3, CLEAN NULL):** the value/π(p)/φ(p) stream drawn as a turtle path across **72 render combos**; Phase-0 planted square fires 5/6 geometry stats; **0 flagged-for-oracle**; 6 shape-stat p<0.01 candidates all fail Bonferroni (2.3×10⁻⁵) as multiple-comparisons noise; G3 base32/onion/lat-long **POINTER_FOUND=False**. **C (X1/X3/X4, NEGATIVE):** **185 enumerated operations** (decimation ×4-phases, cipher-iterate, 4-way interleave, reversal, running-key, prime-value/index/totient keystreams) under the 9-register panel-max bar; all 4 Phase-0 controls recover 1.000 (X3b/X4 held-out 1.000); **0 survivors** — the recovery gate correctly rejected 4 keyed hallucinations (0.91–0.97 recovery, pmax 2.4–2.9 vs bar ~6.4). **X4-totient legitimately re-adjudicates Round-11 N5 under the repaired instrument and re-confirms NEGATIVE.** **D (S3, drop-cap channel, NEGATIVE):** first LP2 drop-cap catalog (15/58 illuminated pages, runes `SLXHXFUMDSFAINGP`); presence/order/gap/centroid/identity all clear no Bonferroni p<0.01 bar (min p=0.18); controls 4/4 + page0=S. **RUNNABLE — page scans are in-repo.** **R (red-team, NO-ERROR-FOUND):** independently reconfirmed B's family-wise 0-survivor (P(≥6)=0.072, noise; the 4.7σ 'components' candidate is a discrete-stat z-tail artifact); no lane silently re-reads a Round-11 negative; no −5.5 relapse; all nulls seed-3301; every lane control-validated. One low-severity non-blocking disclosure gap (B's G3 coord-pair sub-detector ~ Round-11 N2 route (d)) — **fix applied** (B now cites N2). **Standing verdict unchanged: LP2 0–54 remains OTP-class.** What Round 22 shipped is that the "we never even looked at the hinted channels" objection is now **largely closed at the tested resolution** — message-inside-solved-plaintext, numbers-as-direction, koans-as-operations, and art-as-data are all read and clean. The sharpest surviving reopener is **A's printed-line-geometry acrostic** (real page-image line breaks, not the concatenated transliteration this lane had). | `analysis/round22/` (per-lane `PREREG.md`/`RESULTS.md`/`ledger.json`); `LEDGER.json` (R22-A/B/C/D/R, 132→135); `PICKUP-HERE.md` |
| **Round 23** | **The printed-line acrostic — one lane by design (A2, 2026-08-29/30)** | **Round 23 — THE LAST ROADMAP-NAMED REOPENER, READ AT TRUE RESOLUTION: CLEAN NULL.** The Round-23 seed (NEXT-ARMADA-ROADMAP → “ROUND 23 SEED”) judged the honest prior on every remaining hint-channel reopener LOW and authorized exactly one scoped task: **A2**, the first-rune-of-each-PRINTED-LINE (and per-word) acrostic of the 5 rig-solved pages at the TRUE page-image line breaks + word spacing — the single representation R22-A could not build, and the most conventional hiding place a human calligrapher’s self-embedded message would use. Geometry comes from the SAME transcription `tests/validate.py` trusts (`data/scream314_lp.md`, line breaks + `•` word separators preserved); plaintext-on-true-lines built by decrypting each rune IN PLACE with the exact validated transforms (interrupter ᚠ emits nothing, does not advance the key); per-page flat decode matches validate.py on all 5 pages; line counts recovered 10/10/20/8/8. **Phase-0 recovery 2/2 = 1.000** (planted line-first AND word-first acrostics at real page-06 widths recover above their size-matched bars). **Sweep: 48 reads** ({line,word} × {first,last} × {fwd,rev} × {5 pages + concat}), 1000-shuffle structure-preserving null seed 3301, family-wise bar −3.597; readable English ≈ −2.2; best real read −**6.22**. **0 family-wise survivors**; the one per-cell crosser (excess +0.025) is refuted by default — below the 0.048 expected-FP count, gibberish, stable under 5 alternate null seeds, flat on every secondary statistic. No silent re-read (distinct from R11’s ciphertext channels and R22-A’s geometry-free concatenation). **With A2 read, every Phase 1–5 roadmap lens has been read at least once at the tested resolution; the round’s own decision line: natural stopping point — remaining live branches are the PRNG/seed tail and /dev/urandom.** | `analysis/round23/` ([`SYNTHESIS.md`](analysis/round23/SYNTHESIS.md), [`A2-line-acrostic/RESULTS.md`](analysis/round23/A2-line-acrostic/RESULTS.md)); `LEDGER.json` (R23-A2-LINE-ACROSTIC, 135→136) |
| **Round 24** | **Discharge L7-A and L7-B — the two instrument axes no negative ever covered (3-of-8 lanes run, 2026-08-30)** | **Round 24 — FIX THE INSTRUMENT’S TWO KNOWN BLIND SPOTS AND RE-ADJUDICATE: THE NEGATIVES HOLD.** The Gate-#1 planning pass (`analysis/round24/PLAN.md`) killed 5 of 8 candidates before a single decode — C3 long-period polyalphabetic, C4 non-additive feedback, C5 value-stream-as-binary, C6 words-are-the-map, C7 homophonic-downward — each colliding with a positively-measured mechanism exclusion (doublet deficit, refuted autokey diagonals, degenerate nulls, flat IoC). The three survivors ran. **C1 + C1-EXT (the REGISTER axis, L7-A):** the strongest prior B-04 slice — top-40 R20-prior seeds × 16 gens × 4 reductions × sign × atbash × 2 seed-forms = **20,480 decodes** — re-adjudicated under (i) the round16 matched-runic scorer no lane had ever adopted plus an EN/LA/GR rune-trigram panel, and (ii) genuinely language-aware models for Latin, Greek, Old English poetic and Enochian, each control-validated to recover its OWN planted language 2–5× better than the English quadgram scorer (honest-availability call documented: only languages with real in-repo Latin-script mass were modelled). Control reproduces the L7-A gap exactly (planted Latin: matched-power 0.79 vs English 0.50; EN_NOVOWEL 0.00 under both — the matched scorer fixes ORTHOGRAPHY, not language). **Clean NULL both lanes; best matched −6.294 vs family bar −4.685; 0 survivors; the English-only B-04 negatives were NOT hiding a non-English survivor in this slice.** **C2 + C2-EXT (the RELATION axis, L7-B):** built the `pair`/`keyskip2` decoder that CAN represent skip_by_two (plant recovery **100%** where the beam gets 25.8%), calibrated its own claim bar (**7.384** at N=1e6), and re-decoded the top-prior seed slice — **245,975 words, 0 hits, max pmax 5.904 (1.48 below bar), wrong-key null 0/4000 full-gate FPs** — then broadened the closure across **7 axes** (bash `$RANDOM`, perl, tex pgf+lcg, the full sha256_ctr keytext dictionary, non-zero Py2.7 offsets, and the continuous free_drift/drift_at cousin under the drift decoder): **113,432 keys, 8/8 positive controls validated, 0 hits — no beam-based negative reopens over any swept axis** (closest approach 4.6 below its bar). Partial coverage stated per axis; full seed tails out of scope by PREREG. **C8 (date-indexed public pad): UNAVAILABLE at Step 0** — the join key does not exist: exactly 8 clearsigned blocks exist in the canonical corpus, all on prose/index pages; **the unsolved runic pages carry no PGP signature at all**; pipeline control-validated anyway (right-date 1.000 / wrong-date 0.03) and the fullest steelman (630 tests vs the NIST beacon) is null. **Net: the two conditionals hanging over every negative since Round 18 — “only in English” and “only one rejection loop” — are measured through on their strongest slices, and the negatives survive both corrections.** | `analysis/round24/` ([`SYNTHESIS.md`](analysis/round24/SYNTHESIS.md), [`PLAN.md`](analysis/round24/PLAN.md), per-lane `RESULTS.md`); `LEDGER.json` (R24-C1/C1-EXT/C2/C2-EXT/C8, 136→141) |
| **Round 25** | **Compute-tail: the Py2.7-MT 2³² seed grind (owner-elected, PARKED mid-sweep, 2026-08-30/31)** | **Round 25 — GRIND THE LAST RUNNABLE BRANCH, HONESTLY LOW-PRIOR, FULLY CHECKPOINTED.** Not an armada: the owner elected to spend idle compute on the one branch every preceding round agreed was still runnable and untouched — the Python-2.7 MT19937 `init_by_array([w])` 2³² seed tail (reducer random29) — through the R24 skip-aware `pair` decoder and the `hitfn20` three-clause recovery gate (any full-gate clearance is FLAGGED-FOR-ORACLE per R21-L1 and STOPS the sweep). **Parked 2026-08-31 at 0.5015% of 2³² = 21,539,647 words covered, 0 hits, 0 flagged; global best pmax 6.826 vs the pair claim bar 7.384 — deep in noise.** The coverage number is verifiable, not asserted: it reproduces exactly by summing the COMMITTED per-worker checkpoints (`progress_w{0..5}.json` = 20,787,079) + chunk-1’s 506,593 + the 245,975 excluded already-swept baseline. In-band planted-seed control PASSES every invocation (recovery 1.000, HIT=True); a live drill planted a seed in worker 0’s band, fired the HIT path, wrote HIT.json and stopped all 6 workers in 10.8 s. Remaining: ~4.27×10⁹ seeds ≈ 25–32 continuous wall-days at 6-core CPython speed (~1,650–1,966 seeds/s); a GPU/C port of the generator→decoder→scorer pipeline does 2³² in hours and is the single highest-leverage unlock if resumed. A completed null converts “probably not a cheap seeded PRNG” into “the Py2.7 random29 space is not where the pad came from” — it never touches the CSPRNG branch and never moves the OTP-class verdict. Resume: `parallel_grind.py` (per-worker cursors) / `--status`. | `analysis/round25/` ([`SYNTHESIS.md`](analysis/round25/SYNTHESIS.md), [`compute-tail/RESULTS-parked.md`](analysis/round25/compute-tail/RESULTS-parked.md)); `LEDGER.json` (R25-COMPUTE-TAIL-CHUNK1 + parked_state, 141→142) |
| **Round 26** | **The un-run lanes + the semantic seed: fire the built-but-never-swept generators, run the staged keytext sweep, enumerate semantic seeds (4 lanes, 2026-08-31)** | **Round 26 — RELEASE THE SWEEPS THREE INSTRUMENTS WERE BUILT FOR BUT NEVER RAN, AND TEST THE ONE LIVE REFRAME; ALL HARDEN.** Each lane control-validated its instrument BEFORE its sweep counted. **Lane A (un-run generators):** fired the **Perl `rand`** reductions (R19-G2 — was ZERO seeds swept) and the **TeX RNG** generators (G4-TEX — was ZERO decodes) for the first time ever, and swept the three Py2.7 reducers (grb5_mod/grb5_rej/shuffle29) on the axis R21-L3 explicitly left open — **wordsize=64 amd64 `init_by_array`, non-zero keystream offsets, and the `keyskip2` relation** (py27 w64 keystream proven ≠ i386 w32 for string seeds; does NOT touch random29, so the R25 compute-tail tail is uncollided). 8 Perl + 4 TeX + 3 Py2.7 axes, **prior-dense (347 corpus seeds) swept FIRST and empty**, controls recover their own plants at **1.000/HIT**; **0 clears, best pmax 5.438 vs 7.63/7.38 bars.** Row is **partially-run/interim** — the flat-prior dense-from-0 baseline (~42k decodes) was still running at close-out and only extends the same null. **Lane B (staged keytext sweep):** ran R12-C2's fetched-but-never-executed 33-keytext running-key sweep, **skip-aware** (rigid running-key was already doublet-excluded — the reason it must be skip-aware), keyskip1 + keyskip2; control recovers a keytext-running-key plant at **1.000 both presets**; **0/1,562 clears, best pmax 5.470 vs 5.732**, 3 of 9 pages COMPLETE at 4 offsets/text (page 23 to 86%), the large pages honestly CAPPED into `not_covered` under CPU contention (**partially-run**). **Lane C (semantic-seed reframe — the one lane with live-if-small odds):** *the puzzle was built to be solvable, so if 0–54 is a derived key the seed is SEMANTIC, not `/dev/urandom`* — enumerated a bounded **323-value corpus-semantic seed set** (primes, dates, gematria of solved words, hash nibbles, 3301, …) and fed EACH **as a SEED (not a direct key)** through a **7-generator zoo including the totient/prime ladder that reproduces solved page 05**, decoded skip-aware, certified through `hitfn20`. **323 × 7 × 2 = 4,274 rows = 100% of the enumerated set at offset 0**, control min recovery **0.9917**, best screen **−6.578** (flat-random, gap >11.5), **0 escalated, 0 oracle flags (negative).** Even the corpus-endorsed page-56 ladder yields only noise for every semantic seed. **Lane D (red-team, refute-by-default):** **NO-ERROR-FOUND** — independently reproduced all three coverage×power claims from raw rows, confirmed no lane silently re-ran a negative (A first-fires R19-G2/G4-TEX + a distinct R21-L3 axis; B first-executes R12-C2; C distinct from R16-PRNG/R16-KDF/R11), no sieve×panel-max FP-inflation (all lanes use N-scaled panelmax bars, not a fixed bar at large N), every "closed" claim an explicit swept-fraction statement; two MINOR non-fatal hygiene defects noted (a Lane A PREREG-vs-code overlap on 228 w32-identical int-seed rows that reproduces no negative, and Lane A's interim null). **Net: the derived-key branch is HARDENED along three previously un-swept axes — first-ever Perl/TeX decodes, the distinct amd64/offset/keyskip2 Py2.7 slice, the staged keytexts, and the semantic-seed reframe — 0 hits, 0 oracle flags, no stop-and-alert; OTP-class verdict UNCHANGED.** | `analysis/round26/` ([`SYNTHESIS.md`](analysis/round26/SYNTHESIS.md), per-lane `RESULTS.md`, `D/RESULTS.md`); `LEDGER.json` (R26-A/B/C/D, 152→156) |
...
interrupter positions, separators — 7 lenses, all controls PASSED). **This foreclosure is
scoped to those seven lenses only** — the roadmap's un-run lenses (**S3** illustration/drop-cap
channel, **X1–X4** the literal-imperative operations, **G1-turtle** the spatial "direction" render)
are NOT covered by Round 11 and were the Round 22 seed (see
`analysis/NEXT-ARMADA-ROADMAP.md` → "ROUND 22 SEED"). _(Round 22 has since run **ALL SIX**:
**X1/X3/X4** (Lane C), **G1-turtle + G3** (Lane B), **X2** self-embedded read (Lane A), and **S3**
drop-cap channel (Lane D) — every one a control-validated NEGATIVE/clean-null. See the four dated
notes below. The sharpest surviving reopener is A's printed-line-geometry acrostic.)_

> **Updated 2026-08-29 (Round 22 Lane C — X1/X3/X4 executed as OPERATIONS).**
> The three literal imperatives are now **control-validated NEGATIVE** (0 survivors) and no
> longer OPEN: **X1** "do four unreasonable things" (every-4th-rune decimation ×4 phases,
> cipher-iterate ×{2,3,4}, 4-way interleave/de-interleave), **X3** "question all things / test
> the knowledge" (whole-stream reversal re-enciphered forward; solved-plaintext as running key
> over the unsolved stream, both signs), and **X4** "the words OR their numbers" (prime-value /
> prime-index / totient value as a keystream over the letter stream, ±sign ±atbash). 185
> enumerated operations over LP2 0–54, all four Phase-0 plant-and-recover controls PASSED,
> adjudicated by the repaired 9-register panel-max bar + the recovery-gated `is_hit` (never −5.5).
> The size-matched seed-3301 null was empty (0 bar-clears, 0 hits). **Notable:** four keyed
> decodes recovered above 0.90 but were correctly rejected at the panel-max clause — the exact
> hallucination pattern the recovery gate exists to catch. **X4's totient-keystream config
> re-adjudicates Round-11 N5 under the repaired instrument and re-confirms NEGATIVE**, so that
> lens was not a victim of the broken magnet. _(X2 (Lane A) and S3 (Lane D) also ran this round —
> both NEGATIVE; see their dated notes below.)_
> See `analysis/round22/C-literal-imperatives/RESULTS.md`.
> Not covered: non-zero offsets on keyed lanes, interleave/decimation widths ≠ 4, φ(φ(p))/λ(p)/
> digit-plane keystreams (Round 11 N4/N5 territory), decoder constructions outside driftbeam
> exact/drift.

> **Updated 2026-08-29 (Round 22 Lane B — G1-turtle spatial render + G3 coordinate decode).**
> "The numbers are the direction" is now **rendered and control-validated as a CLEAN NULL** (0
> flagged-for-oracle) — no longer OPEN. The value / prime-index π(p) / totient φ(p) stream of LP2
> 0–54 was drawn as a turtle path across **72 render combos** (3 streams × turn-moduli
> {4,6,8,12,29,360} × {absolute-heading, relative-turn} × {unit, value-step}) and scored by a
> 6-stat geometry detector (closure, self-intersections, bbox-fill, caging, symmetry, components)
> against a size-matched histogram-preserving seed-3301 shuffle null (200 surrogates/combo).
> **Phase-0 PASS:** the detector fires on a planted closed square (5/6 stats) and a planted
> square-wave (2 stats), so the null is interpretable. **Every LP2 render is a constrained lattice
> random walk** — no glyph, closed figure, QR/grid, or coordinate string, to the eye or the
> detector. All six shape-stat p<0.01 candidates fail the flag gate (empirical-floor ∧ z-tail
> p<Bonferroni 2.3e-5); the caging/closure departures are value-step order-dependence, not
> figures. **G3 base32/onion/lat-long pointer decode: POINTER_FOUND=False** (low-entropy base32
> run at/below the shuffle null; 0 lat/long; coord-pair rate below null — the coord-pair sub-detector
> re-touches Round-11 **N2 route (d)** with a different statistic, both NEGATIVE, cited per the
> Round-22 red-team). Novel artifact — nobody had rendered LP2's number stream before. _(X2 (Lane A)
> and S3 (Lane D) also ran this round — both NEGATIVE; see their dated notes below.)_ **Not covered
> (reopeners):** non-turtle 2D grid/columnar reads (Lane G2), moduli outside the six, step-functions
> other than unit/value, 3D lifts, L-system rewriting, interrupter-gated pen-up, per-page renders,
> and any English-prose-along-the-path (geometry-only adjudicator).
> See `analysis/round22/B-turtle-spatial-render/RESULTS.md`.

> **Updated 2026-08-29 (Round 22 Lane A — X2 self-embedded read of the SOLVED plaintext).**
> "Discover the truth inside yourself / seek within / test the knowledge" read literally as a
> **selection of the already-decrypted plaintext** (not the ciphertext) is now **control-validated
> NEGATIVE** — no longer OPEN. **3,508 selection functions** (every-k for k∈[2,40] all offsets,
> reversed every-k, diagonal/anti-diagonal folds w∈[2,60]) over the 5 rig-solved pages
> (A WARNING, SOME WISDOM, two KOANs, WELCOME; 1,887 letters / 1,769 rune indices, book order), in
> both a letter stream and a greedy rune-index stream. **Phase-0 PASS:** planted every-7th and
> width-13 diagonal acrostics both recovered as the top selection (recovery 1.000), so the
> instrument provably sees a self-embedded acrostic when present. **0 family-wise survivors**; the 6
> per-cell FPR=0.001 clears (vs 3.5 expected by chance over 3,508 tests) are statistically
> indistinguishable from noise and all unreadable gibberish (best −5.37 vs real English ~−2.2).
> Language-agnostic secondary panel (IoC·N, min-distinct-32, base32 frac, gzip ratio) shows no
> outlier. Instrument finding: width-w main diagonal ≡ every-(w+1) at offset 0, so diagonals are
> subsumed by the every-k family. **Distinct from Round 11** (which worked the CIPHERTEXT
> number/value/separator channels; this works the DECRYPTED English plaintext — red-team confirmed
> no overlap). **Not covered (the sharpest reopener):** first-rune-of-each-PRINTED-LINE acrostics
> using true page-image line-break geometry (only the concatenated transliteration was available —
> physical layout unreconstructed); the wider community-solved LP corpus beyond the 5 rig pages;
> keyword-cued / two-stage selections; non-English target registers.
> See `analysis/round22/A-selfembedded-read/RESULTS.md`.

> **Updated 2026-08-29 (Round 22 Lane D — S3 illumination / drop-cap channel).**
> "Art as data" read as the drop-cap channel is now **control-validated NEGATIVE** — no longer OPEN.
> The first **LP2 drop-cap catalog** was built from all **58 committed page scans** (colour-separable
> large red illuminated glyph — no OCR needed): **15/58** pages illuminated (count not prime, not 29),
> runes reading `SLXHXFUMDSFAINGP`. Presence, page-order, gap structure, centroid, and illuminated-
> rune identity were tested against a size-matched seed-3301 null (10,000 shuffles); **min p = 0.18**,
> nothing clears the p<0.01 Bonferroni bar. **Phase-0 PASS:** 4/4 hand-counted pages + page-0 rune
> identity 'S' reproduced. **RUNNABLE — the page scans are in-repo**, so this was not blocked on
> assets. **Not covered (the reopener):** the **figural-motif** channel (type/count/orientation of
> crosses, trees, the shrouded-corpse figure, mayflies) is documented but NOT coded as data — it
> needs hand-annotation or a black-channel shape pass entangled with the dense-rune OCR floor. Also:
> page 57's illuminated rune is absent from `canonical_pages.json` (1-page identity gap).
> See `analysis/round22/D-illustration-dropcap/RESULTS.md`.
