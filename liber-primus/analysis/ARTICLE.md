...
Here honesty demands a correction. The community had reached this same stream *years* earlier: the Uncovering Cicada wiki published the identical base-60 hex rewrite and even tried reading it as text across several bases — all rubbish — then stopped. So the byte stream was **not** new. What no one had ever done was the *next* step: treat the stream as a cryptographic object — measure its entropy, test it for primality, run it as a key against the runes. That characterization, plus a three-witness adjudicated version with a documented conflict table, is the actual contribution. The stream was on the wall the whole time; nobody had ever asked it the hard questions.
