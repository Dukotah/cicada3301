...
|---|---|---|
| offsets densely scored | **`{A_OFFSETS}`** | **`{B_OFFSETS}`** |
| best `score_norm` | **`{A_BEST}`** | **`{B_BEST}`** |
| bar — `threshold_for(N)` at the **true** trial count | **`{A_THR}`** | **`{B_THR}`** |
| bar — this lane's own ms=8 shuffle null, `null_max + 0.5` | `{A_NULLBAR}` | `{B_NULLBAR}` |
| distance below the binding bar | `{A_GAP}` | `{B_GAP}` |
| positive control, **on the real pad family** | **`{A_CTRL}`** | **`{B_CTRL}`** |
...
|---|---|---|
| **recovery** (>95 % of rune indices, real pads) | **16/16 = 1.000** | `{B_MS3}` |
...
`{B_MS3_NOTE}` This is why ms = 8 was pre-registered for everything and why the whole-ISO view is
...
- Any decode at or above its own `null_max + 0.5` **that also improves** when escalated to full
  ...
  decays. Nothing in either sub-lane came within `{A_GAP}` of the bar.
