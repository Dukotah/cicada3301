...
/* ================= L5-seed32 ADDITIONS (generators 10-13) ==================
 * Each is harness-gated the same way Round 8 gated its ten: it must reproduce a
 * REAL reference implementation exactly before it is swept. See VALIDATION.md.
 *   10  Perl  int(rand(29))       drand48                 -- vs /usr/bin/perl
 *   11  POSIX lrand48() % 29      drand48, 31-bit extract -- vs glibc lrand48
 *   12  Ruby  rand(29)            MT init_genrand + 5-bit mask/reject -- vs ruby
 *   13  xorshift32 % 29           Marsaglia 2003 (13,17,5) -- published recurrence
 * ------------------------------------------------------------------------- */

/* --- drand48 / lrand48 family (Perl's rand, POSIX srand48) --- */
typedef struct { uint64_t x; } d48_t;
static inline void d48_seed(d48_t *d, uint32_t seed)
{ d->x = (((uint64_t)seed << 16) | 0x330EULL) & 0xFFFFFFFFFFFFULL; }
static inline uint64_t d48_step(d48_t *d)
{ d->x = (d->x * 0x5DEECE66DULL + 0xBULL) & 0xFFFFFFFFFFFFULL; return d->x; }

/* --- xorshift32, Marsaglia (2003) "Xorshift RNGs", triple (13,17,5) --- */
typedef struct { uint32_t s; } xs32_t;
static inline void xs32_seed(xs32_t *x, uint32_t seed) { x->s = seed ? seed : 1u; }
static inline uint32_t xs32_next(xs32_t *x)
{ uint32_t v = x->s; v ^= v << 13; v ^= v >> 17; v ^= v << 5; x->s = v; return v; }
...
static void keystream(int gen, uint64_t seed, uint8_t *k, int n)
{
    ...
    case 9: { /* Java new Random(seed).nextInt(29) */
        ...
        break; }
    case 10: { /* Perl: srand(seed); int(rand(29))  == int(drand48()*29) */
        d48_t d; d48_seed(&d, (uint32_t)seed);
        for (int i = 0; i < n; i++) {
            double r = (double)d48_step(&d) / 281474976710656.0;   /* /2^48 */
            int x = (int)(r * N);
            k[i] = (uint8_t)(x >= N ? N - 1 : x);
        }
        break; }
    case 11: { /* POSIX: srand48(seed); lrand48() % 29   (lrand48 = X >> 17) */
        d48_t d; d48_seed(&d, (uint32_t)seed);
        for (int i = 0; i < n; i++)
            k[i] = (uint8_t)(((uint32_t)(d48_step(&d) >> 17)) % N);
        break; }
    case 12: { /* Ruby: srand(seed); rand(29). MT init_genrand + 5-bit mask reject */
        mt_t m; mt_seed(&m, (uint32_t)seed);
        for (int i = 0; i < n; i++) {
            uint32_t v;
            do { v = mt_next(&m) & 31u; } while (v > (uint32_t)(N - 1));
            k[i] = (uint8_t)v;
        }
        break; }
    case 13: { /* xorshift32 (13,17,5) % 29 */
        xs32_t x; xs32_seed(&x, (uint32_t)seed);
        for (int i = 0; i < n; i++) k[i] = (uint8_t)(xs32_next(&x) % N);
        break; }
    default:
...
static const char *GENNAME[] = {
    ...
    "perl int(rand(29)) drand48", "POSIX lrand48()%29",
    "ruby rand(29) MT mask", "xorshift32(13,17,5)%29"
};
#define NGEN 14
...
int main(int argc, char **argv)
{
    ...
    if (argc > 3 && !strcmp(argv[1], "dumpn")) {
        /* L5: bulk keystream dump for the harness gate (>=500 draws x >=3 seeds). */
        int g = atoi(argv[2]);
        uint64_t sd = strtoull(argv[3], 0, 0);
