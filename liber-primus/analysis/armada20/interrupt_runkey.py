...
import run_stats
# raw rune strings per page (need INTERRUPTER chars intact)
raw=open(run_stats.KRIS,encoding="utf-8").read()
segs=[s for s in raw.split("%")]
# build list of (rune_string) for pages, matching load_pages order
def runes_only(s):
...
texts=[("tao","analysis/armada20/taoteching2.txt"),
       ("blake_mhh","analysis/armada20/mhh_45315.txt"),
       ("thomas","analysis/armada20/thomas.txt")]
...
for lab,f in texts:
    ...
    for pi,(seg,r) in enumerate(page_runes):
        ...
        # run interrupter beam on top-4 candidate offsets for this page
...
json.dump([{"label":l,"score":b[0],"detail":b[1]} for l,b in best_overall],
          open("analysis/armada20/test_4_interrupt.json","w"),indent=2)
