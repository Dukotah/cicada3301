...
def load_key(f):
    ...
    return np.array(gp.keyword_to_indices(letters[:60000]))
key=load_key("analysis/foundation/rosicrucian_fama_confessio_key.txt")
print("key idx len",len(key))
glob_best=(-99,None)
allres=[]
for pi,(seg,r) in enumerate(page_runes):
    ...
    allres.append((pi,page_best[0]))
    if page_best[0]>glob_best[0]:
        glob_best=page_best
        print(f"  new best p{pi}: {page_best[0]:.3f} {page_best[1]['pt'][:60]}")
print("GLOBAL BEST interrupter-beam:",glob_best[0])
print(glob_best[1])
json.dump({"best":glob_best[1],"best_score":glob_best[0],"per_page":allres},
          open("analysis/foundation/rosicrucian_interrupt.json","w"),indent=2)
