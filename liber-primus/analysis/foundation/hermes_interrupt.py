...
key=load_key("analysis/foundation/hermes_vol1_key.txt")
print("key len (runes):", len(key))
glob_best=(-99,None)
for pi,(seg,r) in enumerate(page_runes):
    ...
    for mono,sign,atb,o in cand[:4]:
        ...
        if s>glob_best[0]:
            ...
            print(f"  new best p{pi} sign{sign} atb{atb} off{o}: {s:.3f}")
print("FINAL best interrupter-beam score:", round(glob_best[0],3))
print(glob_best[1])
json.dump({"score":glob_best[0],"detail":glob_best[1]},
          open("analysis/foundation/hermes_interrupt.json","w"),indent=2)
