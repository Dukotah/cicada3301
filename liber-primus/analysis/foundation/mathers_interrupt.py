...
key=load_key("analysis/foundation/mathers_letters.txt")
print("key rune-indices:",len(key))
glob_best=(-99,None)
    ...
    for mono,sign,atb,o in cand[:4]:
        ...
        if s>glob_best[0]:
            glob_best=(s,{"page":pi,"sign":sign,"atb":atb,"off":o,
                "n_int":res["n_interrupters"],"pt":res["plaintext"][:120]})
            print(f"  new best p{pi} score={s:.3f} sign={sign} atb={atb} off={o} nint={res['n_interrupters']}")
            print("    ",res["plaintext"][:90])
print()
print(f"[Mathers] best interrupter-beam score_norm: {glob_best[0]:.3f}")
print("   ",glob_best[1])
json.dump({"label":"Mathers-KabbalahUnveiled","score":glob_best[0],"detail":glob_best[1]},
          open("analysis/foundation/mathers_interrupt.json","w"),indent=2)
