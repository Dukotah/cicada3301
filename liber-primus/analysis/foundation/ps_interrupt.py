...
texts=[("PistisSophia-Mead","analysis/foundation/pistis_sophia_letters.txt")]

best_overall=[]
for lab,f in texts:
    key=load_key(f)
    print(f"[{lab}] key len(runes)={len(key)}")
    glob_best=(-99,None)
    for pi,(seg,r) in enumerate(page_runes):
        ...
        if pi%10==0:
            print(f"  page {pi}/{len(page_runes)} cur_best={glob_best[0]:.3f}",flush=True)
...
json.dump([{"label":l,"score":b[0],"detail":b[1]} for l,b in best_overall],
          open("analysis/foundation/ps_interrupt.json","w"),indent=2)
