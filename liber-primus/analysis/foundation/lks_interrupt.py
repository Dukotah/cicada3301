...
key=load_key("analysis/foundation/lks_full_letters.txt")
print("key len(runes):",len(key),"pages:",len(page_runes))
...
for pi,(seg,r) in enumerate(page_runes):
    ...
    for mono,sign,atb,o in cand[:3]:
        ...
        if s>glob_best[0]:
            ...
            print(f"  new best p{pi}: {s:.3f} sign{sign} atb{atb} off{o} nint{res['n_interrupters']}")
            print("    ",res["plaintext"][:90])

print("BEST interrupter-beam:",glob_best[0])
print(glob_best[1])
json.dump({"score":glob_best[0],"detail":glob_best[1]},
          open("analysis/foundation/lks_interrupt.json","w"),indent=2)
