import sys,os,re,json,numpy as np,time
...
lab="thoth_full"; f="analysis/foundation/thoth_letters.txt"
key=load_key(f)
print("key len",len(key),flush=True)
glob_best=(-99,None)
t0=time.time()
for pi,(seg,r) in enumerate(page_runes):
    ...
    pbest=(-99,None)
    for mono,sign,atb,o in cand[:3]:
        stream=[int(x) for x in key[o:o+L]]
        res=solve.find_interrupters(r,stream,sign=sign,atbash=atb,beam_width=200)
        s=res["score_norm"]
        if s>pbest[0]: pbest=(s,(sign,atb,o,res["n_interrupters"],res["plaintext"][:80]))
    if pbest[0]>glob_best[0]:
        glob_best=(pbest[0],{"page":pi,"detail":pbest[1]})
    print(f"p{pi} L={L} best={pbest[0]:.3f} glob={glob_best[0]:.3f} t={time.time()-t0:.0f}s",flush=True)

print("FINAL best interrupter-beam:",glob_best[0],flush=True)
print(glob_best[1],flush=True)
json.dump({"score":glob_best[0],"detail":glob_best[1]},open("analysis/foundation/thoth_interrupt2.json","w"),indent=2)
